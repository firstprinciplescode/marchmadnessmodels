...
ORB_perc_df <- gamelog_comb %>% 
  select(Team, Opp, season, game_id, home_perc_ORB_perc, away_perc_ORB_perc, conf_home_perc_ORB_perc, conf_away_perc_ORB_perc, non_conf_home_perc_ORB_perc, non_conf_away_perc_ORB_perc, def_home_perc_ORB_perc, def_away_perc_ORB_perc, def_conf_home_perc_ORB_perc, def_conf_away_perc_ORB_perc, def_non_conf_home_perc_ORB_perc, def_non_conf_away_perc_ORB_perc)
colnames(ORB_perc_df)[c(5:10)] <- paste0("off_", colnames(ORB_perc_df)[c(5:10)])

ORB_perc_df <- inner_join(final_df, ORB_perc_df, by = c("Team", "Opp", "game_id"))

colnames(ORB_perc_df) <- gsub("%", "", colnames(ORB_perc_df))
...
offense_team <- which(ORB_perc_df$Team == off_string & ORB_perc_df$season == 2025)[1]
defense_team <- which(ORB_perc_df$Opp == def_string & ORB_perc_df$season == 2025)[1]

prediction_df <- cbind(ORB_perc_df[offense_team,c(21:137)], ORB_perc_df[defense_team,c(138:254)], ORB_perc_df[offense_team,c(255:332)], ORB_perc_df[defense_team,c(333:410)], ORB_perc_df[offense_team,c(411:416)], ORB_perc_df[defense_team,c(417:423)])
...
comparison_ORB_perc_func <- function(threshold, year = NULL, locationinput = NULL) {
  ...
  relevant_features <- importance_matrix_ORB_perc$Feature
  current_game <- current_game_vector[relevant_features] %>% as.numeric()
  
  metadata_cols <- reference_df %>%
    select(Team, Opp, season, Date, Location, ORB_perc, off_home_perc_ORB_perc:def_non_conf_away_perc_ORB_perc)
  
  features_matrix <- reference_df %>%
    select(all_of(relevant_features)) %>%
    as.matrix()
  
  weights <- importance_matrix_ORB_perc$Gain
