...
STL_perc_df <- gamelog_comb %>% 
  select(Team, Opp, season, game_id, home_perc_STL_perc, away_perc_STL_perc, conf_home_perc_STL_perc, conf_away_perc_STL_perc, non_conf_home_perc_STL_perc, non_conf_away_perc_STL_perc, def_home_perc_STL_perc, def_away_perc_STL_perc, def_conf_home_perc_STL_perc, def_conf_away_perc_STL_perc, def_non_conf_home_perc_STL_perc, def_non_conf_away_perc_STL_perc)
colnames(STL_perc_df)[c(5:10)] <- paste0("off_", colnames(STL_perc_df)[c(5:10)])

STL_perc_df <- inner_join(final_df, STL_perc_df, by = c("Team", "Opp", "game_id"))

colnames(STL_perc_df) <- gsub("%", "", colnames(STL_perc_df))
...
offense_team <- which(STL_perc_df$Team == off_string & STL_perc_df$season == 2025)[1]
defense_team <- which(STL_perc_df$Opp == def_string & STL_perc_df$season == 2025)[1]

prediction_df <- cbind(STL_perc_df[offense_team,c(21:137)], STL_perc_df[defense_team,c(138:254)], STL_perc_df[offense_team,c(255:332)], STL_perc_df[defense_team,c(333:410)], STL_perc_df[offense_team,c(411:416)], STL_perc_df[defense_team,c(417:423)])
...
comparison_STL_perc_func <- function(threshold, year = NULL, locationinput = NULL) {
  ...
  relevant_features <- importance_matrix_STL_perc$Feature
  current_game <- current_game_vector[relevant_features] %>% as.numeric()
  
  metadata_cols <- reference_df %>%
    select(Team, Opp, season, Date, Location, STL_perc, off_home_perc_STL_perc:def_non_conf_away_perc_STL_perc)
  
  features_matrix <- reference_df %>%
    select(all_of(relevant_features)) %>%
    as.matrix()
  
  weights <- importance_matrix_STL_perc$Gain
